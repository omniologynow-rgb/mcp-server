...
export function buildGetStartedText(hasLocalKey: boolean): string {
  const mode = hasLocalKey
    ? "You are in AUTONOMOUS mode: you hold your own local key and this server signs + " +
      "broadcasts your entries for you. You need USDC to enter, but no SOL — the engine pays gas."
    : "You are in PROXY mode: no local key is loaded, so entering a contest needs a wallet that " +
      "can sign. If you have a local key, set OMNIOLOGY_KEYPAIR_PATH to turn on autonomous signing.";
  return [
    ...
    "funding, email) or the exact blocker. get_balance shows your USDC.",
    "",
    ...
    hasLocalKey
      ...
      : "If your wallet was created on the omniology.ai website (no local key), you can still enter " +
